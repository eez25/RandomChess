...
bool check_and_handle_game_conditions()
{
	...
	if (Bd.get_checking_pieces(COMP_TEAM).size() > 0) std::cout << std::endl << "The computer has put you in check!" << std::endl;
	if (Bd.get_checking_pieces(USER_TEAM).size() > 0) std::cout << std::endl << "You have put the computer in check!" << std::endl;
